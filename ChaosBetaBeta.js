...
if( page.path == '/local/ozo/' || page.path == '/local/beneath/embassy') {
	...
	if (Ochem?.enabledHumors) {
		Ochem.enabledHumors.push('entropy')
		Ochem.enabledHumors.push('surge')
		Ochem.enabledHumors.push('stupidhorrible')
		Ochem.enabledHumors.push('smog')
		Ochem.enabledHumors.push('steel')
		Ochem.enabledHumors.push('silicon')
		Ochem.enabledHumors.push('life')
		Ochem.enabledHumors.push('kivcria')
	}
	if (Ochem?.humorQuantities){
		...
		Ochem.humorQuantities.stupidhorrible = {
        	...
        	party: [
            	['stupidhorrible', 'stupidhorrible', 'stupidhorrible'],
            	['stupidhorrible', 'stupidhorrible', 'stupidhorrible'],
            	['stupidhorrible', 'stupidhorrible', 'stupidhorrible']
        	]
		};
...
env.STATUS_EFFECTS.maddening_ignorance = {
	...
	beneficial: false,
	icon: "https://glass-memoirs.github.io/Chaos-beta/Images/Icons/Metal/maddeningapathy.gif",
...
env.STATUS_EFFECTS.life_resourceful = {
	...
	events: {
		GLOBAL_onDeath: function({originalEventTarget}) {
			...
			let foeTarget = user.enemyTeam.members.sample()
			while (foeTarget == "dead") {
				foeTarget = uesr.enemyTeam.members.sample()
			}
...
env.STATUS_EFFECTS.kivcria_wounds = {
	...
	infinite: true,
	passive: true,
	icon: "https://glass-memoirs.github.io/Chaos-beta/Images/Icons/Kivcria/rotting.gif",
	impulse: {type: "common", component: "kivcria"},
...
//+5% damage per Trot
env.STATUS_EFFECTS.fated_kivcria = {
	slug: "fated_kivcria",
	name: "FATED::KIVCRIA",
	beneficial: true,
	passive: true,
	...
	events: {
		onCreated: function({statusObj}) {
			...
			this.status.power = 0
			if(this.status.affecting?.member?.components) for (const [slotName, slotContents] of Object.entries(this.status.affecting.member.components)) {
				if(slotContents == "kivcria") this.status.power++
			}

			if(this.status.affecting?.member?.augments) for (const augmentSlug of this.status.affecting.member.augments) {
				let augment = env.ACTOR_AUGMENTS.generic[augmentSlug]
				if(augment?.component) if(augment.component[1] == "kivcria") this.status.power += 2
			}
		},
		GLOBAL_onBeforeCombatHit: function(context) {
			if(!this.status.power || context.amt > 0 || !this.status.affecting.team.members.includes(context.origin)) return;
			if(context.origin == this.status.affecting) {
				...
				if(context.amt > 0 && rotCount && !context.beneficial) {
					this.status.outgoingMult = (0.05 * this.status.power) * rotCount 
				} else {
					this.status.outgoingMult = 0
				}
			}
		...
		GLOBAL_onBeforeAddStatus: function(context) {
			if(!this.status.power || context.amt > 0 || !this.status.affecting.team.members.includes(context.origin)) return;
...
env.STATUS_EFFECTS.kivcria_wall = {
	...
	name: "Wall Rot",
	beneficial: false,
	icon: "https://glass-memoirs.github.io/Chaos-beta/Images/Icons/Kivcria/WallRot.gif",
	passive: "modifier",
	infinite: true,
	events: {
		onBeforeAction: function(context) {
...
env.STATUS_EFFECTS.kivcria_festering = {
	...
	passive: "modifier",
	infinite: true,
	icon: "https://glass-memoirs.github.io/Chaos-beta/Images/Icons/Kivcria/Festering.gif",
	outgoingMult: 0,
	incomingMult: 0,
...
env.COMBAT_ACTORS.rot_bearer_foe = {
	...
	portrait: `<img class="portrait" src="https://glass-memoirs.github.io/Chaos-beta/Images/Actors/Freak-face1_mimic.gif">`,
	portraitUrl: 'https://glass-memoirs.github.io/Chaos-beta/Images/Actors/Freak-face1_mimic.gif',
	graphic: `
		<div class="sprite-wrapper archival-golem golemsprite daemonsprite intrusiveactor" id="%SLUG-sprite-wrapper">
