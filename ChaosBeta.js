...
env.STATUS_EFFECTS.forte = {
	...
	help: "'repeat next action for the amount of Forte'\n'if the action is either HARMONY or SCALE, repeat 8 times'", 
	//[]^[] < reading this what the fuck was i on
	//absolutely no fucking adhd meds thats what dickshit. this is still glass btw
	events: {
		onTurn: function() { 
			reactDialogue(this.status.affecting, 'surge') 
			delete this.status.justGotSurge
...
env.ACTIONS.player_law = { //Funky little move, had to change it up just like momentum.
	...
	exec: function(user, target) {
		...
		if (check("modList").includes("morehumors")) {
...
env.ACTIONS.life_tuvazu = {
	slug: "life_tuvazu",
	name: "Tuvazu Imports",
	type: "target+autohit", //look this is done just to not run an autohit thing ok.
	details: {
